...
function verifyToken(token: string): boolean {
  ...
  const payload = `freecell:${timestamp}`;
...
export async function PUT(request: Request) {
  ...
  const existing = await adminDb.collection('freecell').where('username', '==', sanitized).get();

  if (existing.empty) {
    await adminDb.collection('freecell').add({ username: sanitized, wins: 1 });
  } else {
